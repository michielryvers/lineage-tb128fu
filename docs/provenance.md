...
Device-specific changes cover modern Android configuration, legacy vendor compatibility, snapshot/recovery setup, C2D dependency isolation, and the OpenGL UI default. Kernel changes include the maintained SM6225 merge, Lenovo wake IPC preservation, UART vote ownership, Wi-Fi bounds/CSA handling, gesture read EOF, FCM configuration, EROFS support task/group utilization clamping and the upstream trace-marker stream-open fix (`2972e3050e3517a85ca1813b227d4c302e804343`, John Keeping; original authorship retained). The common device power policy adds the measured 36% event clamp with the original frequency floor; see [performance evidence](performance-results.md). Platform patches remain small reviewable files under `rom-build/patches/`.
...
The September 20 platform patches preserve cached charging/screen state across PowerStatsAggregator.reset and apply optional SFMainPostBootPolicy after compositor boot initialization. The common device fork supplies the opt-in cpuset. Their measured scope and limitations are documented in [compositor results](compositor-results.md).
